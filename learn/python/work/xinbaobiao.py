...
tempFilePath = "/data/select/xbb_" + yesterday.strftime("%Y-%m-%d") + ".xls"
...
for index in range(0, len(tempFileData)):
    ...
    if index + 1 > len(tempFileData) - 3:
        tempFileData[index] = round(float(tempFileData[index]), 3)
    # print tempFileData[index + 1]
    # 数据与样式处理
    default_ws.cell(row=taskRow, column=column).value = tempFileData[index]
